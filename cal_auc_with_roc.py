"""
VAE异常检测ROC曲线绘制脚本
计算VAE重建误差和潜在特征传统检测器的AUC，并绘制ROC曲线
"""
...
def plot_roc_curves(labels, scores_dict, title="ROC曲线对比",
                    save_path="./result/vae_roc_curves.png"):
    """
    绘制多条ROC曲线
    ...
    print(f"ROC曲线数据已保存到: {data_path}")

    return results

def plot_detailed_analysis(labels, scores, method_name, save_dir="./result/detailed_analysis"):
    """
    绘制单方法的详细ROC分析
    ...
    # 保存图像
    save_path = os.path.join(save_dir, f"{method_name.lower()}_detailed_analysis.png")
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # 注意：重建误差值越大表示越异常，所以直接使用（正值）
    # 传统auc函数使用负值是因为它假设评分值越大越正常
    ...
    X = torch.cat([mean_tensor, logvar_tensor], dim=1).cpu().numpy()

    # 1. LOF（局部离群因子）检测器
    print("训练LOF检测器...")
    lof = LocalOutlierFactor(n_neighbors=min(500, len(X)), novelty=False)
    lof.fit(X)
    lof_score = lof.negative_outlier_factor_  # 值越小越异常

    # 2. 单类SVM检测器
    print("训练单类SVM检测器...")
    svm = OneClassSVM(gamma='scale', nu=0.1).fit(X)
    svm_score = svm.score_samples(X)  # 值越小越异常

    # 3. 椭圆包络（EllipticEnvelope）检测器
    print("训练椭圆包络检测器...")
    cov = EllipticEnvelope(random_state=0, contamination=0.1).fit(X)
    cov_score = cov.decision_function(X)  # 值越小越异常

    # 4. 孤立森林（IsolationForest）检测器
    print("训练孤立森林检测器...")
    iso = IsolationForest(random_state=0, contamination=0.1).fit(X)
    iso_score = iso.score_samples(X)  # 值越小越异常

    # 计算传统检测器AUC（使用原始评分，因为原始评分值越大越正常）
    lof_auc = roc_auc_score(test_labels, lof_score)
    svm_auc = roc_auc_score(test_labels, svm_score)
    cov_auc = roc_auc_score(test_labels, cov_score)
    iso_auc = roc_auc_score(test_labels, iso_score)

    print(f"\n传统检测器AUC:")
    print(f"  LOF AUC: {lof_auc:.4f}")
    print(f"  单类SVM AUC: {svm_auc:.4f}")
    print(f"  椭圆包络 AUC: {cov_auc:.4f}")
    print(f"  孤立森林 AUC: {iso_auc:.4f}")
    ...
    scores_dict = {
        ...
        'LOF': -lof_score,               # 取负：值越小越异常 → 值越大越异常
        'OneClassSVM': -svm_score,       # 取负：值越小越异常 → 值越大越异常
        'EllipticEnvelope': -cov_score,  # 取负：值越小越异常 → 值越大越异常
        'IsolationForest': -iso_score    # 取负：值越小越异常 → 值越大越异常
    }
    ...
    roc_results = plot_roc_curves(
        ...
        title="VAE异常检测方法ROC曲线对比",
        save_path="./result/vae_roc_comparison.png"
    )
    ...
    for method_name, scores in scores_dict.items():
        ...
        detailed_info = plot_detailed_analysis(
            labels=labels,
            scores=scores,
            method_name=method_name,
            save_dir="./result/detailed_analysis"
        )
        detailed_results[method_name] = detailed_info
    ...
    with open("./result/vae_with_roc_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"完整结果已保存到: ./result/vae_with_roc_results.json")

    # -------------------- 最佳阈值应用示例 --------------------
    print("\n=== 最佳阈值应用示例（以VAE-MAE为例） ===")

    # 以VAE-MAE为例，展示如何使用最佳阈值
    mae_scores = scores_dict['VAE-MAE']
    mae_best_threshold = detailed_results['VAE-MAE']['best_threshold']

    # 使用最佳阈值进行预测
    predictions = (mae_scores >= mae_best_threshold).astype(int)

    print(f"VAE-MAE最佳阈值: {mae_best_threshold:.4f}")
    print("混淆矩阵:")
    ...
    print("\n=== 完成 ===")
    print("ROC曲线和相关分析图已保存到 ./result/ 目录")
    print("运行完成！")
